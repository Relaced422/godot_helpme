...
func setup_sprite() -> void:
	...
	sprite.pixel_size = 0.005  # ← SMALLER value = SMALLER sprite (try 0.003-0.008)
	sprite.texture_filter = BaseMaterial3D.TEXTURE_FILTER_NEAREST
	
	# Scale the sprite down
	sprite.scale = Vector3(0.5, 0.5, 0.5)  # ← Scale to 50% (try 0.3 to 0.8)
	sprite.position.y = 1.0  # ← Lift sprite up (adjust this value)
...
func update_sprite_direction(move_direction: Vector3) -> void:
	...
	# Get camera direction for relative sprite selection
	...
	# Choose sprite based on dominant direction
	if abs(forward_dot) > abs(right_dot):
		if forward_dot > 0:
			sprite.texture = sprite_back  # Moving away from camera
		else:
			sprite.texture = sprite_front  # Moving toward camera
	else:
		if right_dot > 0:
			sprite.texture = sprite_right  # Moving right
		else:
			sprite.texture = sprite_left  # Moving left
...
func move_forward(steps: int) -> void:
	...
	if current_tile.has_method("get") and current_tile.get("tile_type") == BoardTile.TileType.REVERSE:
		is_reverse = true
		steps = -steps
		print("🔄 Reversing movement! Going back ", abs(steps), " spaces")
	
	is_moving = true
	movement_started.emit()
	
	await move_step_by_step(steps, tiles)
	
	is_moving = false
	movement_finished.emit()
	landed_on_tile.emit(current_tile_index)

## Move step by step with hop (updated to handle negative steps)
func move_step_by_step(total_steps: int, tiles: Array) -> void:
	...
	# Notify board about landing (after all steps complete)
